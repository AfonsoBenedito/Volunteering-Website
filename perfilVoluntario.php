<?php
    ...
    $comando = "SELECT * FROM voluntariosVC19 v WHERE (v.Username = '$Username');";
...
<html lang="en">
    ...
    <body>
        ...
        <main>
            ...
            <!-- <div class="perfilIncompleto">
                <h3>Informações de utilizador por preencher 'Editar perfil > Alterar Dados'</h3>
            </div> -->
                ...
                <li class="zonasPerfil">
                    ...
                    <div class="zonaDescricaoPerfil">
                        <ul>
                            ...
                            <a href="editarPerfil.php" class="editarPerfil">Editar perfil</a>
                        </ul>
                        <ul id="idade">VOLUNTARIO - X ANOS</ul>
                        <ul id="interesses">INTERESSES</ul>
                        <ul id="biografia">BIOGRAFIA</ul>
                    </div>
                </li>
            </div>
...
$today = new DateTime();
$birthdate = new DateTime($row[8]);
$interval = $today->diff($birthdate);
$age = $interval->format('%y Anos');
echo "<script> document.getElementsByClassName('nomePerfil')[0].innerText = '$row[4]';";
echo "document.getElementsByClassName('nomePerfil')[0].innerText += ' ' + '$row[5]';";
echo "document.getElementById('idade').innerText = 'Voluntario - ' + '$age';";
echo "document.getElementById('interesses').innerText = 'Interesses: ' + '$row[14]';";
echo "document.getElementById('biografia').innerText = 'Biografia: ' + '$row[13]';";
echo "document.getElementById('fotoPerfilImg').src = '$row[16]';";
echo "</script>";


if($row[2] == $_SESSION["User"] and isset($_SESSION["User"])){
    
    echo "<script>";
    echo "document.getElementsByClassName('editarPerfil')[0].style.visibility = 'visible';";
    echo "</script>";
    ...
    if ($row[7] == NULL){
        array_push($arrayIncompleto, 'Género');
    }
    if ($row[9] == NULL){
        array_push($arrayIncompleto, 'Carta de Condução');
    }
    if ($row[10] == NULL){
    ...
    if ($row[13] == NULL){
        array_push($arrayIncompleto, 'Biografia');
    }
    if ($row[14] == NULL){
        array_push($arrayIncompleto, 'Intresses');
    }
    if ($row[15] == NULL){
        array_push($arrayIncompleto, 'População Alvo');
    }
